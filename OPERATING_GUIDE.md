...
**Status:** **Live in production: https://nj-parcel-flood.pages.dev** (Cloudflare Pages + R2, §6.2/§7.1). Guide-Phase 7 (web app) closed out — 5 views (Search & Map, Jurisdiction Summary, District Exposure, Ranked Municipalities, Methodology), global filters, CSV export, live P8 address geocoding, a full visual-design pass (custom teal color system, elevation, type scale, risk-band color reused beyond the map — driven by two `design:design-critique` rounds). Municipality-choropleth boundary matching 564/564. `09_validate.py` (§12.1) — 6 of 7 QA gates PASS; the one FAIL is the already-known, owner-approved join-rate limitation (88.34%, PROGRESS.md 2026-08-12). Axe/Playwright suite (§12.2): **16/16 passing**, run repeatedly and reliably (dev-server only by design). Deploying for real caught 2 genuine production-only bugs dev testing couldn't have: MapLibre's worker script never shipped in the prod build at all, and the real fix needed its own sibling-chunk import resolved too (a first attempt looked complete — file shipped, right MIME type — but the worker still died silently; caught precisely via `page.on('worker')` showing `created` then `closed` within milliseconds, zero console error). Both root-caused and fixed, verified live via real tile traffic on the production URL, not assumed. **Parcel detail-level toggle's zoom-out floor now genuinely reaches Municipality's own floor (9), not just the button** — `parcels.pmtiles` widened from z13 to z9 (owner: "full scope" after a cheaper attempt turned out to be a real correctness bug, not a style choice — see PROGRESS.md), with explicit per-tier layer-visibility switching in `MapCanvas.tsx` so the existing municipality choropleth isn't regressed for anyone not in Parcel mode. Solved two real environment blockers along the way (Docker Desktop's WSL2 backend stuck, and a Windows Application Control policy blocking pyogrio natively) without touching any security policy — routed the GDAL-dependent pipeline step through WSL Ubuntu instead. Full detail: PROGRESS.md 2026-08-22 and 2026-08-13 (search "Live production deployment", "Parcel zoom-out fix"). **Known limitation, owner-approved to carry forward, not resolved:** statewide MOD-IV join rate 88.34%, below the required ≥97% gate. **Standing tooling caveat** (owner-reviewed, non-blocking): this session's preview pane can't composite frames, stalling anything MapLibre needs `requestAnimationFrame` for — doesn't leave product behavior unverified; a real browser (Playwright, or direct checks against the live URL) confirms it instead. Next: portfolio assets (screenshots, `CASE_STUDY.md`); optional custom domain for the Pages deployment.
