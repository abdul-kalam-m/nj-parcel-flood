...
**Status:** Guide-Phase 7 (web app, §7.2) built — all 4 views, filters, CSV export, on the locked stack (§6.2). Found and fixed a major retroactive bug along the way: `klokantech/tippecanoe` never actually wrote PMTiles for *either* tileset despite guide-Phase 6 reporting them complete — both were silently MBTiles (SQLite) mislabeled with a `.pmtiles` extension; fixed with a real `go-pmtiles convert` step, both tilesets re-verified with correct `PMTi` magic bytes. Extensively verified live (choropleth, all 3 data views with real numbers, search, CSV export). One piece (parcel-click detail panel against the newly re-tiled `parcels.pmtiles`) couldn't complete live re-verification twice, both times traced to the preview pane not compositing frames — **owner reviewed and directed treating the existing binary/tool-level verification as sufficient** (see PROGRESS.md 2026-08-13, "Owner decision" entry); closed, not an open item. `09_validate.py` (§12.1, guide-Phase 8) run for real, statewide, separately — **6 of 7 QA gates PASS**; the one FAIL is `join_rate=0.8834`, the already-known, owner-approved-to-carry-forward limitation. Municipality-choropleth boundary matching closed 553/564 → **564/564** (caught and reverted a real regression from a too-general abbreviation rule along the way — see PROGRESS.md 2026-08-13). **Known limitation, owner-approved to carry forward, not resolved:** statewide MOD-IV join rate 88.34%, below the required ≥97% gate (see PROGRESS.md 2026-08-12). Next: P8 live geocoding; axe/Playwright suite (§12.2); R2 upload; rest of guide-Phase 8 (README, portfolio assets).
