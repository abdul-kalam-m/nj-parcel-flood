...
**Status:** **Live in production: https://nj-parcel-flood.pages.dev** (Cloudflare Pages + R2). Guide-Phase 7 (web app: 5 views, live P8 geocoding, full visual-design pass) and guide-Phase 8 (QA + portfolio assets) both closed out. Municipality-choropleth boundary matching 564/564. `09_validate.py`: **6 of 7 QA gates PASS** — the one FAIL is the known, owner-approved MOD-IV join-rate limitation (88.34%, PROGRESS.md 2026-08-12). Axe/Playwright suite: **15/15 passing**, real-browser verified. Portfolio assets (screenshots, pipeline-stats, `CASE_STUDY.md`, QR code) complete in `8. NJ FLOOD RISK DASHBOARD\case-study-assets\` (the Drive/portfolio location, not this repo). Full build history — including two production-only bugs found only by deploying for real, and a same-day feature revert on UX grounds after the fix itself worked — is in PROGRESS.md; search by date rather than relying on this line to carry it all. **Known limitation, carried forward, not resolved:** statewide MOD-IV join rate 88.34%, below the required ≥97% gate. **Standing tooling caveat** (non-blocking): this session's preview pane can't composite frames for anything MapLibre needs `requestAnimationFrame` for — real-browser checks (Playwright, or the live URL directly) cover it instead. **The app has never rendered raw FEMA/NJDEP flood-hazard-zone geometry as its own layer**, only computed risk derived from it (PROGRESS.md 2026-08-22). Next: decide whether a raw flood-zone overlay layer is wanted as a real feature; optional custom domain for the Pages deployment.
